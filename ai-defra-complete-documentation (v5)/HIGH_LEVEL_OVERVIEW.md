...
| Capability | Detail |
|---|---|
| Chat with documents | Ask questions in natural language; on the first turn, the system searches your selected knowledge group files for relevant passages and uses them to ground the answer |
| Draw on LLM general knowledge | The AI model can also answer from its general training knowledge (up to its training date), not just from uploaded documents |
...
| Multi-turn conversations | Follow-up questions work — the system maintains conversation context across turns |
