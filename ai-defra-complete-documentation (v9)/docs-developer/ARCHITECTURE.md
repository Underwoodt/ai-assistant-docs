...
- Generates embeddings via AWS Bedrock (Titan Embed v2, 1024-dim, model ID: `amazon.titan-embed-text-v2:0`)
...
### Loading Spinner for Async GenAI Requests
...
Key steps in the async flow:

1. Frontend `POST /chat { question, conversation_id, model_id, knowledge_group_ids }` to Agent (with `user-id` header)
2. Agent saves message with `status: queued`, enqueues `{ message_id, conversation_id, question }` to SQS, returns `202 { conversation_id, message_id, status: queued }` immediately
3. Frontend redirects to conversation view; begins polling `GET /start/{conversationId}` → `GET /conversations/{conversationId}`
4. SQS Worker polls continuously; dequeues message, conditionally performs RAG lookup, calls `converse(history, system_prompt, model)` on Bedrock, persists response to MongoDB, deletes processed SQS message
5. On next poll, frontend receives completed message and updates Redis cache
...
### Detailed Ingest Journey

The following sequence diagrams provide a step-by-step view of the upload and ingestion journeys, covering the full flow from the upload screen through to embedding storage.
...
The data models below illustrate the persistence layer underpinning the RAG flow and show how the Agent Service and Knowledge Service interact at a data level.
