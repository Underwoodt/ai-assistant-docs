...
sequenceDiagram
    ...
    U->>CDP: Upload file
    CDP->>S3: Store file
    CDP->>FE: Callback (upload complete, s3Key, uploadReference)
    FE->>KN: POST /knowledge-groups/{id}/documents {s3_key, cdp_upload_id, metadata}
    KN->>MDB: Save Document (status: not_started)

    Note over KN: asyncio.create_task() per document
    KN->>S3: Fetch file bytes
    loop For each chunk (800 chars, 100 overlap)
        KN->>BD: invoke Titan Embed v2 (chunk text)
        BD-->>KN: 1024-dim embedding vector
        KN->>VDB: INSERT KnowledgeVector
    end
    KN->>MDB: Update Document (status: ready / failed, chunk_count)

    U->>FE: Poll document status
    FE->>KN: GET /knowledge-groups/{id}/documents/{docId}
    KN-->>FE: {status: ready}
    FE-->>U: Show document as ready to query
```

---

## 3. RAG Search (Internal — Agent → Knowledge)
...
sequenceDiagram
    ...
    VDB-->>KN: Top-n matching KnowledgeVector rows
...
| All services | AWS (Bedrock, S3, SQS) | IAM Role (prod) / LocalStack dummy credentials (local) |
